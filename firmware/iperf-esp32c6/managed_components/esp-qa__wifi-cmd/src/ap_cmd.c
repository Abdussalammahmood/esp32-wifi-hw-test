...
static ap_deauth_args_t ap_deauth_args;
static int cmd_do_ap_deauth(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **) &ap_deauth_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, ap_deauth_args.end, argv[0]);
        return 1;
    }
    uint16_t aid = 0;
    if (ap_deauth_args.aid->count > 0) {
        aid = ap_deauth_args.aid->ival[0];
    }
    esp_err_t err = esp_wifi_deauth_sta(aid);
    LOG_WIFI_CMD_DONE(err, "AP_DEAUTH");
    return 0;
}

void wifi_cmd_register_ap_deauth(void)
{
    if (!ap_deauth_args.end) {
        ap_deauth_args.aid = arg_int0("a", "aid", "<aid>", "aid of station, default all");
        ap_deauth_args.end = arg_end(2);
    }
    esp_console_cmd_t ap_death_cmd = {
        .command = "ap_deauth",
        .help = "Deauth station from AP",
        .hint = NULL,
        .func = &cmd_do_ap_deauth,
        .argtable = &ap_deauth_args
    };
    ESP_ERROR_CHECK(esp_console_cmd_register(&ap_death_cmd));
}
