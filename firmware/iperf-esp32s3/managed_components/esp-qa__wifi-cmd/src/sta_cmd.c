...
#if WIFI_CMD_SCAN_PARAMETERS_SUPPORTED
typedef struct {
    struct arg_str *action;       /* set | reset | get */
    struct arg_int *max;          /* active scan time max (ms) */
    struct arg_int *min;          /* active scan time min (ms) */
    struct arg_int *passive_time; /* passive scan time (ms) */
    struct arg_int *dwell;        /* home channel dwell time (ms, uint8_t) */
    struct arg_end *end;
} sta_scan_parameters_args_t;
static sta_scan_parameters_args_t scan_parameters_args;

static void wifi_cmd_print_scan_parameters(const wifi_scan_default_params_t *p)
{
    ESP_LOGI(APP_TAG, "scan parameters: active.min=%" PRIu32 " active.max=%" PRIu32
             " passive=%" PRIu32 " dwell=%u",
             p->scan_time.active.min, p->scan_time.active.max,
             p->scan_time.passive, p->home_chan_dwell_time);
}

static int do_scan_parameters_get(void)
{
    wifi_scan_default_params_t params = {0};
    esp_err_t err = esp_wifi_get_scan_parameters(&params);
    if (err == ESP_OK) {
        wifi_cmd_print_scan_parameters(&params);
    }
    LOG_WIFI_CMD_DONE(err, "GET_SCAN_PARAMETERS");
    return 0;
}

static int do_scan_parameters_reset(void)
{
    esp_err_t err = esp_wifi_set_scan_parameters(NULL);
    LOG_WIFI_CMD_DONE(err, "RESET_SCAN_PARAMETERS");
    return 0;
}

static int do_scan_parameters_set(void)
{
    wifi_scan_default_params_t params = {0};
    esp_err_t err = 0;

    if (scan_parameters_args.max->count > 0) {
        params.scan_time.active.max = (uint32_t)scan_parameters_args.max->ival[0];
    }
    if (scan_parameters_args.min->count > 0) {
        params.scan_time.active.min = (uint32_t)scan_parameters_args.min->ival[0];
    }
    if (scan_parameters_args.passive_time->count > 0) {
        params.scan_time.passive = (uint32_t)scan_parameters_args.passive_time->ival[0];
    }
    if (scan_parameters_args.dwell->count > 0) {
        params.home_chan_dwell_time = (uint8_t)scan_parameters_args.dwell->ival[0];
    }

    err = esp_wifi_set_scan_parameters(&params);
    LOG_WIFI_CMD_DONE(err, "SET_SCAN_PARAMETERS");
    if (err == ESP_OK) {
        wifi_cmd_print_scan_parameters(&params);
    }
    return 0;
...
static int cmd_do_scan_parameters(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **) &scan_parameters_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, scan_parameters_args.end, argv[0]);
        return 1;
    }

    const char *action = scan_parameters_args.action->sval[0];
    if (strcmp(action, "get") == 0) {
        return do_scan_parameters_get();
    } else if (strcmp(action, "reset") == 0) {
        return do_scan_parameters_reset();
    } else if (strcmp(action, "set") == 0) {
        return do_scan_parameters_set();
    }

    ESP_LOGE(APP_TAG, "unknown action '%s', expect: set | get | reset", action);
    return 1;
}

void wifi_cmd_register_sta_scan_parameters(void)
{
    if (!scan_parameters_args.end) {
        scan_parameters_args.action       = arg_str1(NULL, NULL, "<set|get|reset>", "action to perform");
        scan_parameters_args.max          = arg_int0(NULL, "max", "<int>", "active scan time max (ms), used by 'set'");
        scan_parameters_args.min          = arg_int0(NULL, "min", "<int>", "active scan time min (ms), used by 'set'");
        scan_parameters_args.passive_time = arg_int0(NULL, "passive-time", "<int>", "passive scan time (ms), used by 'set'");
        scan_parameters_args.dwell        = arg_int0(NULL, "dwell", "<30-150>", "home channel dwell time (ms), used by 'set'");
        scan_parameters_args.end          = arg_end(2);
    }
    const esp_console_cmd_t scan_parameters_cmd = {
        .command  = "scan_parameters",
        .help     = "WiFi scan parameters: 'set' (with --max/--min/--passive-time/--dwell), "
        "'reset' to IDF defaults (passes NULL to esp_wifi_set_scan_parameters), "
        "'get' to print current values",
        .hint     = NULL,
        .func     = &cmd_do_scan_parameters,
        .argtable = &scan_parameters_args,
    };
    ESP_ERROR_CHECK(esp_console_cmd_register(&scan_parameters_cmd));
}
#endif /* WIFI_CMD_SCAN_PARAMETERS_SUPPORTED */
...
static int cmd_do_sta_scan(int argc, char **argv)
{
    ...
    if (scan_args.bssid->count > 0) {
        if (wifi_cmd_str2mac(scan_args.bssid->sval[0], bssid) != ESP_OK) {
    ...
    esp_err_t err = scan_args.null->count > 0 ?
                    esp_wifi_scan_start(NULL, block_scan) :
                    esp_wifi_scan_start(&scan_config, block_scan);
...
static void wifi_cmd_register_sta_scan(void)
{
    if (!scan_args.end) {
        ...
        scan_args.null = arg_lit0(NULL, "null", "Call esp_wifi_scan_start(NULL, false)");
        ...
        scan_args.block = arg_lit0(NULL, "block", "Block scan");
...
#if WIFI_CMD_STA_WAKE_TIME_SUPPORTED
typedef struct {
    struct arg_int *unicast_wake_time;
    struct arg_int *broadcast_wake_time;
    struct arg_int *keep_alive_time;
    struct arg_end *end;
} sta_wake_time_args_t;
static sta_wake_time_args_t sta_wake_time_args;

static int cmd_do_sta_wake_time(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **) &sta_wake_time_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, sta_wake_time_args.end, argv[0]);
        return 1;
    }
    if (sta_wake_time_args.unicast_wake_time->count > 0) {
        esp_wifi_set_sleep_min_active_time(sta_wake_time_args.unicast_wake_time->ival[0]);
    }
    if (sta_wake_time_args.broadcast_wake_time->count > 0) {
        esp_wifi_set_sleep_wait_broadcast_data_time(sta_wake_time_args.broadcast_wake_time->ival[0]);
    }
    if (sta_wake_time_args.keep_alive_time->count > 0) {
        esp_wifi_set_keep_alive_time(sta_wake_time_args.keep_alive_time->ival[0]);
    }
    ESP_LOGI(APP_TAG, "WAKE_TIME: OK");
    return 0;
}

void wifi_cmd_register_sta_wake_time(void)
{
    if (!sta_wake_time_args.end) {
        sta_wake_time_args.unicast_wake_time = arg_int0("u", "unicast_wake_time", "<int>", "unicast wake time (ms)");
        sta_wake_time_args.broadcast_wake_time = arg_int0("b", "broadcast_wake_time", "<int>", "broadcast wake time (ms)");
        sta_wake_time_args.keep_alive_time = arg_int0("k", "keep_alive_time", "<int>", "keep alive time (ms)");
        sta_wake_time_args.end = arg_end(2);
    }
    const esp_console_cmd_t sta_wake_time_cmd = {
        .command = "sta_wake_time",
        .help = "WiFi is station mode, set wake time (unicast/broadcast/keep_alive)",
        .hint = NULL,
        .func = &cmd_do_sta_wake_time,
        .argtable = &sta_wake_time_args
    };
    ESP_ERROR_CHECK(esp_console_cmd_register(&sta_wake_time_cmd));
}
#endif
