...
{
    if (!strncmp(sec_channel, "none", 5)) {
        return WIFI_SECOND_CHAN_NONE;
    } else if (!strncmp(sec_channel, "above", 6)) {
        return WIFI_SECOND_CHAN_ABOVE;
    } else if (!strncmp(sec_channel, "below", 6)) {
        return WIFI_SECOND_CHAN_BELOW;
    } else {
        ESP_LOGE(APP_TAG, "Can not get second channel from str: %s", sec_channel);
    }
    return WIFI_SECOND_CHAN_NONE;
}
